...
## 1. AUTORIDAD ABSOLUTA: SUPABASE AUTH

En OLA SOCIAL, **Supabase Auth** es la única autoridad de identidad permitida. No existen tablas de contraseñas paralelas, tokens falsificados ni sesiones simuladas.

```
+-------------------+        +--------------------+        +---------------------+
|   Usuario en Web  | -----> | Cloudflare Turnstile| -----> | Supabase Auth       |
| (Login / Registro)|        | (Validación Humana)|        | (Google OAuth / OTP)|
+-------------------+        +--------------------+        +---------------------+
                                                                      |
                                                                      v
                                                           +---------------------+
