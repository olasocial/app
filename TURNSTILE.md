...
### Despliegue de la Edge Function en Supabase:
Para desplegar la función `verify-turnstile` en el proyecto Supabase (`ocyjnplyywvctqikjrrh`), se debe ejecutar:
```bash
npx supabase functions deploy verify-turnstile --project-ref ocyjnplyywvctqikjrrh --no-verify-jwt
```
> **Nota Crítica sobre `--no-verify-jwt`:**
> La verificación humana ocurre **antes** de que el usuario inicie sesión. Por tanto, la petición procede de un cliente no autenticado con la clave anon. Si no se pasa el parámetro `--no-verify-jwt`, Supabase rechazará la llamada con HTTP 401 Unauthorized.
>
> **Nota sobre el error 404 / CORS:**
> Si la función no ha sido desplegada en el proyecto de Supabase, el API Gateway de Supabase responderá con `404 NOT_FOUND` y sin los headers de preflight CORS esperados por el navegador, lo que se manifiesta en la consola como un error de CORS en la petición preflight de OPTIONS. Desplegar la función con el comando anterior resuelve de inmediato el error.
